        ...
        loop {
            ...
            match self.stream.next()? {
                ...
                vt::Token::Ctrl(ch) => match ch {
                    '\0' => return Some(Input::Keyboard(kbmod::CTRL | vk::SPACE)),
...
impl<'input> Stream<'_, '_, 'input> {
    ...
    fn handle_bracketed_paste(&mut self) -> Option<Input<'input>> {
        ...
        if !self.parser.bracketed_paste {
            Some(Input::Paste(mem::take(
                &mut self.parser.bracketed_paste_buf,
            )))
        } else {
            None
        }
    ...
    fn parse_xterm_mouse(params: &[u16], final_byte: char) -> Option<Input<'input>> {
        ...
        mouse.modifiers |= if (btn & SHIFT) != 0 {
            kbmod::SHIFT
        } else {
            kbmod::NONE
        };
        mouse.modifiers |= if (btn & ALT) != 0 {
            kbmod::ALT
        } else {
            kbmod::NONE
        };
        mouse.modifiers |= if (btn & CTRL) != 0 {
            kbmod::CTRL
        } else {
            kbmod::NONE
        };
