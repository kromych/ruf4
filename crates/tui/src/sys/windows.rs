...
pub fn switch_modes() -> io::Result<()> {
    unsafe {
        ...
        match check_bool_return(Console::SetConsoleMode(
            ...
            Err(e) if e.kind() == io::ErrorKind::InvalidInput => Err(io::Error::other(
                "This application does not support the legacy console.",
            )),
            other => other,
        }?;
...
pub fn preferred_languages<'a>(arena: &'a Arena) -> BVec<'a, &'a str> {
    ...
    res.extend_sloppy(
        arena,
        langs.split_terminator('\0').filter(|s| !s.is_empty()),
    );
    res
}
