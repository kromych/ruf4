...
const KBMOD_FOR_WORD_NAV: InputKeyMod = if cfg!(any(target_os = "macos", target_os = "ios")) {
    kbmod::ALT
} else {
    kbmod::CTRL
};
...
impl Tui {
    ...
    pub fn indexed_alpha(
        ...
        self.framebuffer
            .indexed_alpha(index, numerator, denominator)
    }
    ...
    fn render_styled_text(
        ...
        if overflow == Overflow::Clip || target_width >= actual_width {
            self.framebuffer
                .replace_text(target.top, target.left, target.right, text);
        } else {
            ...
            self.framebuffer
                .replace_text(target.top, target.left, target.right, &modified);
        }
...
impl<'a> Context<'a, '_> {
    ...
    fn table_move_focus(&mut self, prev_key: InputKey, next_key: InputKey) {
        ...
        let mut prev_next = NodeSiblings {
            prev: None,
            next: None,
        };
        ...
        let Some(focused_next) = prev_next
            .get(siblings_idx)
            .or_else(|| container.borrow().children.get(children_idx))
        else {
            return;
        };
    ...
    pub fn styled_label_end(&mut self) {
        {
            ...
            let cursor =
                unicode::MeasurementConfig::new(&content.text.as_bytes()).goto_visual(Point {
                    x: CoordType::MAX,
                    y: 0,
                });
    ...
    pub fn scrollarea_end(&mut self) {
        ...
        if !self.input_consumed {
                ...
                && container_rect.contains(self.tui.mouse_position)
            {
                ...
                .tui
                .is_subtree_focused_alt(container_id, container_depth)
    ...
    pub fn menubar_menu_begin(&mut self, text: &str, accelerator: char) -> bool {
        let accelerator = if cfg!(any(target_os = "macos", target_os = "ios")) {
            '\0'
        } else {
            accelerator
        };
    ...
    pub fn menubar_menu_end(&mut self) {
            ...
            && matches!(key, vk::ESCAPE | vk::UP | vk::DOWN)
        {
            if matches!(key, vk::UP | vk::DOWN) {
                ...
                if self
                    .tui
                    .is_node_focused(ln.parent.map_or(0, |n| n.borrow().id))
                {
                    let selected_next = if key == vk::UP {
                        ln.children.last
                    } else {
                        ln.children.first
                    };
    ...
    fn menubar_shortcut(&mut self, shortcut: InputKey) {
        let val = shortcut.value() & 0x00FFFFFF; // strip modifiers
        let is_fkey = (0x70..=0x87).contains(&val);
        let is_letter = (val as u8 as char).is_ascii_uppercase();

        if is_letter || is_fkey {
            ...
            if is_fkey {
                shortcut_text.push(self.arena(), 'F');
                let fnum = val - 0x70 + 1;
                if fnum >= 10 {
                    shortcut_text.push(self.arena(), (b'0' + (fnum / 10) as u8) as char);
                }
                shortcut_text.push(self.arena(), (b'0' + (fnum % 10) as u8) as char);
            } else {
                shortcut_text.push(self.arena(), val as u8 as char);
            }
...
const INVALID_STYLED_TEXT_CHUNK: StyledTextChunk = StyledTextChunk {
    offset: usize::MAX,
    fg: StraightRgba::zero(),
