...
pub fn parse_f64_approx(input: &[u8]) -> Option<f64> {
    ...
    if pos < input.len() && (input[pos] == b'e' || input[pos] == b'E') {
        ...
        while pos < input.len() && input[pos].is_ascii_digit() {
            exp_val = exp_val
                .saturating_mul(10)
                .saturating_add((input[pos] - b'0') as i32);
            pos += 1;
        }
...
mod tests {
    ...
    fn fail(s: &str) {
        assert!(
            parse_f64_approx(s.as_bytes()).is_none(),
            "expected None for {:?}",
            s
        );
    }
    ...
    fn approx(s: &str, expected: f64) {
        let got = p(s);
        let diff = (got.to_bits() as i64)
            .wrapping_sub(expected.to_bits() as i64)
            .unsigned_abs();
        assert!(
            diff <= 1,
            "more than 1 ULP off for {:?}: got={}, expected={}",
            s,
            got,
            expected
        );
