...
impl Arena {
    pub const fn empty() -> Self {
        Self::Owned {
            arena: release::Arena::empty(),
        }
    }

    pub fn new(capacity: usize) -> io::Result<Self> {
        Ok(Self::Owned {
            arena: release::Arena::new(capacity)?,
        })
    }
    ...
    pub(super) fn delegated(delegate: &release::Arena) -> Self {
        ...
        Self::Delegated {
            delegate: unsafe { &*(delegate as *const _) },
            borrow,
        }
    }
...
impl Allocator for Arena {
    unsafe fn realloc(
        ...
        unsafe {
            self.delegate_target()
                .realloc(old_ptr, old_size, new_size, align)
        }
    }
