...
impl<'a> RegexParser<'a> {
    ...
    fn parse_char_class(&mut self) -> Result<Regex, String> {
        ...
        fn unexpected_unicode(c: char) -> Result<Regex, String> {
            Err(format!(
                "non-ASCII character '{c:?}' not supported in character class"
            ))
        }
...
impl<'a, 'c> CodeGen<'a, 'c> {
    ...
    fn emit(
        ...
        match regex {
            ...
            Regex::EndOfLine => {
                let if_node = self.compiler.alloc_iri(IRI::If {
            ...
            Regex::Group { inner, capturing } => {
                if *capturing {
                    ...
                    let save_end = self.compiler.alloc_iri(IRI::Mov {
                        dst: end_reg,
                        src: off_reg,
                    });
                    ...
                    self.captures
                        .push(self.compiler.arena, (start_reg, end_reg));

                    let save_start = self.compiler.alloc_iri(IRI::Mov {
                        dst: start_reg,
                        src: off_reg,
                    });
    ...
    fn emit_repeat(
        ...
        if min == 0 && max == u32::MAX && matches!(*inner, Regex::Dot) {
            let off_reg = self.compiler.get_reg(Register::InputOffset);
            let skip_node = self.compiler.alloc_iri(IRI::MovImm {
                dst: off_reg,
                imm: u32::MAX,
            });
    ...
    fn emit_charset(
        ...
        if max == 1 {
            ...
            for slot in &mut chars {
                let Some(mut idx) = cs.get_and_reset_lowest() else {
                    break;
                };
