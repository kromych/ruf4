...
impl<'a> Backend<'a> {
    ...
    fn linear_scan_allocation(
        ...
        let mut available: Vec<Register> = (Register::FIRST_USER_REG..Register::COUNT)
            .rev()
    ...
    fn dst_by_node(&mut self, ir: IRCell<'a>) -> usize {
        ...
        if off != usize::MAX {
            ...
            self.relocations
                .push(Relocation::ByNode(self.assembly.instructions.len(), ir));
            0
        }
    }
    ...
    fn dst_by_name(&mut self, name: &'a str) -> usize {
        match self.functions_seen.get(name) {
            ...
            None => {
                self.relocations
                    .push(Relocation::ByName(self.assembly.instructions.len(), name));
                0
            }
        }
    }
...
impl<'a> LivenessAnalysis<'a> {
    ...
    fn dfs(
        ...
        match ir.instr {
            ...
            IRI::If {
                condition: Condition::Cmp { lhs, rhs, .. },
                ..
            } => {
    ...
    fn compute_intervals(&self) -> Vec<LiveInterval> {
        ...
        let mut intervals: Vec<LiveInterval> = vreg_ranges
            .into_iter()
            .map(|(vreg_id, (start, end))| LiveInterval {
                vreg_id,
                start,
                end,
            })
