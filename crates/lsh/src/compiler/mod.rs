...
impl<'a> Compiler<'a> {
    pub fn new(arena: &'a Arena) -> Self {
        ...
        let mut s = Self {
            ...
            highlight_kinds: vec![HighlightKind {
                identifier: "other",
                value: 0,
            }],
    ...
    fn alloc_vreg(&self) -> IRRegCell<'a> {
        ...
        self.arena
            .alloc_uninit()
            .write(RefCell::new(IRReg::new(id)))
    }
    ...
    fn intern_highlight_kind(&mut self, identifier: &str) -> &HighlightKind<'a> {
        let idx = match self
            .highlight_kinds
            .binary_search_by(|hk| hk.identifier.cmp(identifier))
        {
            Ok(idx) => idx,
            Err(idx) => {
                let identifier = arena_clone_str(self.arena, identifier);
                let value = self.highlight_kinds.len() as u32;
                self.highlight_kinds
                    .insert(idx, HighlightKind { identifier, value });
                idx
            }
        };
...
impl<'a> HighlightKind<'a> {
    pub fn fmt_camelcase(&self) -> HighlightKindCamelcaseFormatter<'a> {
        HighlightKindCamelcaseFormatter {
...
impl<'a, 's> IRChainBuilder<'a, 's> {
    ...
    fn build(&self) -> IRSpan<'a> {
        self.span
            .unwrap_or_else(|| IRSpan::single(self.compiler.alloc_noop()))
    }
}
